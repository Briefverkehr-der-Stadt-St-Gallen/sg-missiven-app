...
declare 
    %templates:wrap
    %templates:default("field", "document")
    %templates:default("sort", "send-date")
...
declare function app:show-hits($node as node(), $model as map(*)) {
    let $subtypeSubmitted := request:get-parameter("subtype", ())
    let $subtypes := if ($subtypeSubmitted eq 'document') then ('edition', 'regest') else $subtypeSubmitted
