...
declare function capi:list($request as map(*)) {
    ...
    let $template := 
        if ($templateAvail and $works?mode = 'browse' and false()) then 
            $templatePath
        else
...
function capi:list-works($root as xs:string?, $cached, $params as map(*)) {
    (: session:clear(), :)
    let $sort := request:get-parameter("sort", "send-date")
    ...
    return (
        ...
        session:set-attribute($config:session-prefix || ".field", request:get-parameter("subtype", "text")),
