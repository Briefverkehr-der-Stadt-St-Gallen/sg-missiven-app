...
declare function dapi:source($request as map(*)) {
    let $id := xmldb:decode($request?parameters?id)
    let $doc := if ($config:address-by-id and not(contains($id, '.'))) then "data/" || $id || '.xml' else $id
    return
        if ($doc) then
        ...
        if ($xml?data) then
            ...
            let $mapped :=
                if ($request?parameters?map) then
                    ...
                    $xml?data
            let $data :=
                if (empty($request?parameters?xpath) and $request?parameters?("user.highlight") and exists(session:get-attribute($config:session-prefix || ".search"))) then
