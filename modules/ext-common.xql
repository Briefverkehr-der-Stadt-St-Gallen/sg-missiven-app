...
declare function pmf:format-date-correspAction($date as node()*) {
    if (matches($date/text(), "^\d{4}$")) then
        format-date(xs:date($date/text() || '-01-01'), "[Y]", 'de', (), ())
    else if (matches($date/text(), "^\d{4}-\d{2}$")) then
        format-date(xs:date($date/text() || '-01'), "[Y] [MNn]", 'de', (), ())
    else
        format-date(xs:date($date/text()), '[Y] [MNn] [D1]', 'de', (), ())
};
...
declare function pmf:get-name-list($ids as xs:string*) {
    let $actor := for $id in $ids return $config:register-person/id($id)
    let $names := $actor/*[@type eq 'main']
    return $names
};
