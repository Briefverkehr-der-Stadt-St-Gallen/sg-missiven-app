...
(:~~
 ...
 :)
declare variable $config:webcomponents :="2.14.0";
...
declare variable $config:facets := [
    map {
        ...
        "output": function($label) {
            let $person := $config:register-person/id($label)/tei:*[@type='main']/text()
            return 
                if ($person) then ($person) else ($label)
        }
    },
    map {
        "dimension": "recipient",
    ...
    map {
        "dimension": "exhib-place",
        "heading": "Austellungsort",
        "select": map {
            "source": "api/facets/exhib-place"
        },
        "output": function($label) {
    ...
    map {
        ...
        "output": function($label) {
            let $keyword := $config:register-keyword/id($label)/tei:label[@type='main'][@xml:lang='de']/text()
            return 
                if ($keyword) 
                then ($keyword)
                else ($label)
 ...
 (:    map {
        "dimension": "send-date",
        "heading": "browse.orig-date",
        "select": map {
            "source": "api/facets/send-date"
        }
    },
    map {
        "dimension": "date",
...
(:~
 ...
 :)
declare variable $config:data-root := "/db/apps/stgm-data";
...
declare variable $config:register-keyword := doc($config:data-root || "/keyword/keyword.xml");
...
declare variable $config:iiif-base-uri := 'https://media.sources-online.org/cantaloupe/iiif/2/sg-missiven!';
